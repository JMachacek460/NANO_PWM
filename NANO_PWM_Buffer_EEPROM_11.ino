...
struct ControlState {
  ...
  unsigned long errorCount = 0;               // celkovy pocet erroru od startu nebo *CLS
  byte countHigh = 0, countLow = 0;           // pocet period aktualniho stavu
};
...
void zobrazNastaveni() {
  ...
  Serial.print(F("Max duty cycle -s: "));  Serial.print(aktualniNastaveni.max_strida);  Serial.println(F(" permille"));
...
bool zpracujUniverzalniPrikaz(const char *command, unsigned int minValue, unsigned int maxValue, byte *lowPtr, byte *highPtr, unsigned int *uLowPtr = nullptr, unsigned int *uHighPtr = nullptr) {
  ...
  bool rangeValid = (nove_spodni >= minValue && nove_spodni <= maxValue && nove_horni >= minValue && nove_horni <= maxValue);
  ...
  if (mSerial.isNewData) {
    ...
    while (mSerial.state != 255) {
      ...
      if (strncmp(mSerial.cmdPtr, "*idn?", 5) == 0) {
        //*IDN?
        Serial.print(reinterpret_cast<const __FlashStringHelper *>(TEXT_IDN));
        Serial.print(F(" Version: "));
        Serial.println(aktualniNastaveni.verze);
        mSerial.state = 255;  //ukonci dekodovani mSerial.bufferu
      }
      if (strncmp(mSerial.cmdPtr, "*rst", 4) == 0) {
        //*RST
        Serial.print(reinterpret_cast<const __FlashStringHelper *>(TEXT_RST));
        Serial.print(F(" Version: "));
        Serial.println(aktualniNastaveni.verze);
        tovarniNastaveni();
        zobrazNastaveni();
